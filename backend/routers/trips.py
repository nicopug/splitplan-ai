    ...
    # Usiamo OpenAI per collegarci a OpenRouter
    ...
    # Modello scelto (Gemini 2.0 Flash su OpenRouter è veloce ed economico/gratis)
    AI_MODEL = "google/gemini-2.0-flash-001" 

    if OPENROUTER_API_KEY:
        print(f"[OK] Loaded OpenRouter API Key: {OPENROUTER_API_KEY[:5]}***")
        ai_client = OpenAI(
            base_url="https://openrouter.ai/api/v1",
            api_key=OPENROUTER_API_KEY,
            default_headers={
                "HTTP-Referer": "https://splitplan.vercel.app", 
                "X-Title": "SplitPlan AI",
            }
        )
    else:
        print(f"[ERROR] OPENROUTER_API_KEY not found. Funzionerò in modalità Mock/Manuale.")
    ...
    def generate_proposals(trip_id: int, prefs: PreferencesRequest, session: Session = Depends(get_session), current_account: Account = Depends(get_current_user)):
        ...
        # --- LOGICA INTELLIGENTE CON OPENROUTER ---
        if ai_client:
            try:
                prompt = f"""
                ...
                # Pulizia JSON (toglie ```json e ``` se presenti)
    ...
    def generate_itinerary_content(trip: Trip, proposal: Proposal, session: Session):
        """Genera l'itinerario usando AI (OpenRouter) + Dati Reali da OpenStreetMap"""
    ...
    def vote_proposal(
        proposal_id: int, 
        score: int, 
        # user_id lo calcoliamo noi per sicurezza
        ...
        # 2. IDENTIFICAZIONE AUTOMATICA (Smart Fix)
        ...
        # 3. Gestione Voto (Inserimento o Aggiornamento)
    ...
    def chat_with_ai(trip_id: int, req: ChatRequest, session: Session = Depends(get_session), current_account: Account = Depends(get_current_user)):
        ...
        IMPORTANTE: Usa la cronologia sopra per capire il contesto.
        ...
        try:
            if ai_client:
                # Chiamata OpenRouter per la chat
                completion = ai_client.chat.completions.create(
                    model=AI_MODEL,
                    messages=[
