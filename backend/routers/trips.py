...
def vote_proposal(
    ...
    # Se user_id è fornito (modalità test/demo), usiamo quello, altrimenti cerchiamo tramite account
    participant = None
    if user_id:
    ...
    if total_voters >= trip.num_people:
        ...
        session.add(trip)
        session.commit()
        # Niente generazione itinerario qui per evitare timeout
...
def simulate_votes(trip_id: int, session: Session = Depends(get_session), current_account: Account = Depends(get_current_user)):
    ...
    if not proposals: raise HTTPException(status_code=400)
