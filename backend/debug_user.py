...
def check_and_reset_user():
    with Session(engine) as session:
        ...
        if not results:
            ...
            for u in all_users:
                logger.info(
                    f"User: {u.name} {u.surname}, Email: {u.email}, Premium: {u.is_subscribed}, Calendar: {u.is_calendar_connected}"
                )
            return
        ...
        for user in results:
            ...
            logger.info(
                f"Stato attuale - Premium: {user.is_subscribed}, Calendar: {user.is_calendar_connected}"
            )
